...
public class ChooseImageGenerator implements ICaptcha {
    ...
    static {
        ...
        new FontUtil();
        File imageDir = new File(FontUtil.tmpDir + "/image/");
        imageDirPaths = imageDir.list();

        for (int i = 0; i < imageDirPaths.length; i++) {
            images.put(imageDirPaths[i], new File(FontUtil.tmpDir + "/image/" + imageDirPaths[i]).list());
        }
    }
    ...
    public Captcha generateCaptcha(int size, int width, int height, float fontSize) {
        ...
        Font charFont = FontUtil.charFont.deriveFont(0, fontSize);
    ...
    public static String[] getRandImagePath(int validDir) {
        ...
        if (imageDirPaths != null) {
            ...
            // 取出有效的图片目录
            // 取出目录下随机的valid个图片路径
            String[] imagePaths = new File(classPath + "image/" + imageDirPaths[validDir]).list();
            ...
            for (int i = 0; i < VALID; i++) {
                ...
                imagePath[i] = classPath + "image/" + imageDirPaths[validDir] + "/" + imagePaths[validImageIndex[i]];
            }
        ...
        for (String key : images.keySet()) {
            if (!key.equals(validDir)) {
                ...
                for (int j = 0; j < im.length; j++) {
                    images_.add(classPath + "image/" + key + "/" + im[j]);
                }
            }
        }
